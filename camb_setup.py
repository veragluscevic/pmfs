#This module has functions that setup for a camb run needed to create the density power spectra.
...
def make_camb_ini(zs, out_root=MAIN_PATH+'matter_power/', kmax=2):
    ...
    fout.write('#Parameters for CAMB\n')
    fout.write('#output_root is prefixed to output file names\n')
    fout.write('output_root = %s\n' % out_root)
    fout.write("""  
...
l_sample_boost          = 1
    ...
    fout.write('transfer_num_redshifts  = %i\n' % len(zs))
    fout.write('transfer_interp_matterpower = T\n')
    sorted_zs = np.sort(zs)[::-1]
    for i,z in enumerate(sorted_zs):
        fout.write('transfer_redshift(%i)    = %f\n' % (i+1,z))
    fout.write('#Matter power spectrum output against k/h in units of h^{-3} Mpc^3\n')
    for i,z in enumerate(sorted_zs):
        fout.write('transfer_matterpower(%i) = matterpower_%i.dat\n' % (i+1,i))
    np.savetxt('%s/zs.txt' % out_root,sorted_zs)
