...
\caption{Illustration of the quadrupolar pattern of 21-cm emission from the last ($B$-dependent) term of \eq{\ref{eq:tbsoln}} in the frame of the emitting atoms. Lower panels correspond to increasingly larger magnetic field (marked on each panel in comoving units), where the bottom panel corresponds to the saturated case. Notice how the type of quadrupole differs for the no-magnetic field case and the case the field is ``strong'' in the saturation sense. \label{fig:hp}}
\end{figure}
