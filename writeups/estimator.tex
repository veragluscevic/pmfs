...
We now derive an unbiased minimum-variance quadratic estimator $\widehat B$ for a uniform magnetic field $\vec B$. We start by noting that the redshifted 21-cm brightness temperature Fourier modes $T(\vec k)$ contain contribution from the noise $T^N(\vec k)$ and the signal $T^S(\vec k)$, where the signal is generated both by the 21-cm signal with no magnetic field (null-case signal $T^S_0(\vec k)$), and by the magnetic field, 
\beq
\bga
...
\beq
\bga
\frac{\partial T_0^S}{\partial B}(\vec k)\equiv  \delta(k)\frac{\partial G}{\partial B}(\widehat k,B=0),\\
\frac{\partial G_0}{\partial B}(\widehat k)\equiv\frac{\partial G}{\partial B}.(\widehat k,B=0)
\ega
\label{eq:dTdB_dGdB}
\eeq
...
\bga
\langle T(\vec k)T^*(\vec k')\rangle = P_\text{null}(\vec k)(2\pi)^3\delta_D(\vec k-\vec k')\\
 + 2BP_{\delta}(\vec k)Re\left[G_0^*(\widehat k)\frac{\partial G_0}{\partial B}(\widehat k)\right] (2\pi)^3\delta_D(\vec k-\vec k').
\ega
\label{eq:TT_step2}
\eeq
...
\beq
\widehat B_{\vec k} = \frac{\frac{1}{V}T(\vec k)T^*(\vec k) - P_\text{null}(\vec k)}{2P_{\delta}(\vec k)Re\left[G_0^*(\widehat k)\frac{\partial G_0}{\partial B}(\widehat k)\right]}.
\label{eq:hatBk}
\eeq 
...
= 
\frac{\left\langle\left(\frac{1}{V}T(\vec k)T^*(\vec k) - P_\text{null}(\vec k)\right)\left(\frac{1}{V}T^*(\vec k')T(\vec k') - P_\text{null}(\vec k')\right)\right\rangle}{4P_{\delta}(\vec k)P_{\delta}(\vec k')Re\left[G_0^*(\widehat k)\frac{\partial G_0}{\partial B}(\widehat k)\right]Re\left[G_0(\widehat k')\frac{\partial G_0^*}{\partial B}(\widehat k')\right]},
\ega
\label{eq:mean_BB}
\eeq
...
\beq
\langle \widehat B_{\vec k}\widehat B^*_{\vec k'}\rangle = \frac{P_\text{null}^2(\vec k)\left(\delta_{\vec k,\vec k'}  + \delta_{\vec k,-\vec k'} \right)}{\left(2P_{\delta}(\vec k)Re\left[G_0^*(\vec k)\frac{\partial G_0}{\partial B}(\vec k)\right]\right)^2},
...
\beq
\bga
\widehat B = \frac{\sum_{\vec k}\frac{\frac{1}{V}T(\vec k)T^*(\vec k) - P_\text{null}(\vec k)}{P_\text{null}^2(\vec k)}P_{\delta}(\vec k)Re\left[G_0^*(\vec k)\frac{\partial G_0}{\partial B}(\vec k)\right]}{{\sum_{\vec k}\left(\frac{2P_{\delta}(\vec k)Re\left[G_0^*(\vec k)\frac{\partial G_0}{\partial B}(\vec k)\right]}{P_\text{null}(\vec k)}\right)^2}},
\ega
...
\beq
\bga
\sigma^2_{\widehat B} = \left(\sum_{\vec k}\left(\frac{2P_{\delta}(\vec k)Re\left[G_0^*(\vec k)\frac{\partial G_0}{\partial B}(\vec k)\right]}{P_\text{null}(\vec k)}\right)^{2}\right)^{-1}.
\ega
...
\times\left[ P_\delta(\vec k')G_0^*(\vec k')\frac{\partial G_0}{\partial B_i}(\vec k') - P_\delta(\vec k)G_0(\vec k)\frac{\partial G_0^*}{\partial B_i}(\vec k)\right],
\ega
\eeq
...
\begin{widetext}
\beq
P^N_{B_i}(\vec K) = \left(\frac{(2\pi)^3}{2V}\sum_{\vec k} \frac{\left|P_\delta(\vec k')G_0^*(\vec k')\frac{\partial G_0}{\partial B_i}(\vec k') - P_\delta(\vec k)G_0(\vec k)\frac{\partial G_0^*}{\partial B_i}(\vec k)\right|^2}{\left(G^2_0(\vec k)P_\delta(\vec k) + P^N(\vec k)\right)\left(G^2_0(\vec k')P_\delta(\vec k') + P^N(\vec k')\right) } \right)^{-1},
\label{eq:NK}
\eeq
\end{widetext}
