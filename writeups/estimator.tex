...
\beq
\bga
\sigma^2_{\widehat B} = \left(\sum_{\vec k}\left(\frac{2P_{\delta}(\vec k)Re\left[G_0^*(\vec k)\frac{\partial G_0}{\partial B}(\vec k)\right]}{G^2_0({\bf{\widehat k}})P_\delta(k)+P^N(\vec k)}\right)^{2}\right)^{-1}.
\ega
