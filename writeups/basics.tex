...
The redshifted 21-cm signal can be represented as a function of the location in physical space $I(\vec{r})$, or in Fourier space $\widetilde{I}(\vec{k})$. In sky coordinates, these functions become $\mathcal{I}(\theta_x, \theta_y, \theta_\nu)$, and $\widetilde{\mathcal{I}}(u,v,\eta)$. Here, vector $\vec{k}$ (in the units of comoving Mpc$^{-1}$) is a Fourier dual of $\vec{r}$ (comoving Mpc), and likewise, $\theta_x$ (rad), $\theta_y$ (rad), and $\theta_\nu$ (Hz) are duals of the coordinates in the $uv$-plane $u$ (rad$^{-1}$), $v$ (rad$^{-1}$), and $\eta$ (seconds), respectively. These two sets of coordinates are related through linear transformations in the following way
\begin{equation}
\begin{gathered}
...
\eeq
where the proportionallity factor is a Jacobian $\frac{d\theta_xd\theta_ydf}{dr_xdr_ydr_z}$. Note also that there is no scaling between $I$ and $\mathcal{I}$, as they are the same function, up to a change of variables.
...
\label{eq:visibility}
\eeq
Note that here, $\theta_\nu$ is a discrete variable such that $\theta_{\nu,\text{max}}-\theta_{\nu,\text{min}}=\Delta\nu$ is the bandwidth of the observed data cube centered on $z$ (see also Appendix \ref{app_Vrms}).
%%%%%%%%%%%%%%
\subsection{Power spectra and noise}
\label{subsec:noise}

Let us first define a brightness-temperature power spectrum,
\beq
\langle \widetilde{I}(\vec{k})\widetilde{I}^*(\vec{k}')\rangle \equiv (2\pi)^3P_{\widetilde{I}}\delta(\vec{k}-\vec{k}').
\label{eq_tildeI_power}
\eeq
Furthermore, let us note that the observable we wish to relate to is the visibility function---a complex Gaussian variable with a zero mean, whose noise-induced variance is derived in Appendix \ref{} and reads
\beq\bga
...
\int e^{2\pi i \theta_\nu(\eta-\eta')}d\theta_\nu =\delta(\eta-\eta'),
\eeq
is the periodic delta-function on the $t_1$ interval.  Taking into account the scaling relation of Eq.~(\ref{eq_tilde_I_vs_Ik_scaling}), introducing the power spectrum of Eq.~(\ref{eq_tildeI_power}), and keeping in mind that $\delta(ax)=\frac{1}{a}\delta(x)$, we arrive at
\beq
...
\label{eq:nk}
\eeq
where $N_\text{ant}$ is the number of antennas in the array, and the integration is done on the half of the $uv$ plane (because the visibility has the following property $V(u,v,f)=V^*(-u,-v,f)$, and only half the plane contains independent samples). We assume that the array consists of many antennas, so that time-dependence of $n(u,v)$ is negligible; if this is not the case, one should compute its time average to account for Earth's rotation.

Let us now derive $n(\vec k)$ for a specific array configuration that is of particualr interest to cosmology---a tightly packed array of simple dipole antennas, tiling a squared-surface of the area $(\Delta L)^2$ with a filling fraction close to one. This is a design such as the FFT telescope described in \cite{Tegmark and Zaldarriaga (2009)}. In this case, the beam is close to the size of the entire survey and coveres 1 sr, the affective area of a single dipole is $A_e = \lambda^2$, and the effective number of antennas is then $N_\text{ant} = \frac{(\Delta L)^2}{\lambda^2}$.
